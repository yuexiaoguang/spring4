 ...
 * 除此之外, {@code MockHttpServletResponse}也与Servlet 3.1的{@code setContentLengthLong()}方法兼容.
 */
public class MockHttpServletResponse implements HttpServletResponse {
	...
	/**
	 * 返回是否已设置字符编码.
	 * <p>如果是{@code false}, {@link #getCharacterEncoding()}将返回默认编码值.
	 */
	...
	/**
	 * 返回所有指定header的名称.
	 * <p>从Servlet 3.0开始, 这个方法也定义了HttpServletResponse.
	 * 
	 * @return header名称{@code Strings}的{@code Set}, 或空{@code Set}
	 */
	@Override
	 ...
	 * 从Spring 3.1开始, 它返回Servlet 3.0兼容的字符串值.
	 * 考虑使用{@link #getHeaderValue(String)}进行原始对象访问.
	 * 
	 * @param name header的名称
	 * 
	 * @return 关联的header值, 或{@code null}
	 */
	@Override
	 ...
	 * 返回给定header的所有值.
	 * <p>从Servlet 3.0开始, 此方法也在HttpServletResponse中定义.
	 * 从Spring 3.1开始, 它返回一个Servlet 3.0兼容的字符串值列表.
	 * 考虑使用{@link #getHeaderValues(String)}进行原始对象访问.
	 * 
	 * @param name header的名称
	 * 
	 * @return 关联的header值, 或空列表
	 */
	@Override
	...
	/**
	 * 默认实现委托给{@link #encodeURL}, 按原样返回给定的URL.
	 * <p>可以在子类中重写, 以特定于重定向的方式追加会话ID等.
	 * 对于常规URL编码规则, 请改写常用的{@link #encodeURL}方法, 应用于重定向URL以及常规URL.
	 */
	@Override
	...
	/**
	 * 内部类, 一旦超出缓冲区大小, 就会调整ServletOutputStream以将响应标记为已提交.
	 */
	...
	/**
	 * 内部类, 一旦超出缓冲区大小, 就会调整PrintWriter以将响应标记为已提交.
	 */
