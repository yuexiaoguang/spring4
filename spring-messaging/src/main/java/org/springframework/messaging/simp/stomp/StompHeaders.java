 ...
 * <ul>
 * <li>{@link #getFirst(String)}返回header名称的第一个值</li>
 * <li>{@link #add(String, String)}添加到header名称的值列表中</li>
 * <li>{@link #set(String, String)}将header名称设置为单个字符串值</li>
 * </ul>
 */
public class StompHeaders implements MultiValueMap<String, String>, Serializable {
	...
	/**
	 * 是否启用心跳.
	 * 返回{@code false}, 如果{@link #setHeartbeat}设置为"0,0", 否则{@code true}.
	 */
	...
	/**
	 * 获取 destination header.
	 * 适用于SEND, SUBSCRIBE, 和MESSAGE帧.
	 */
	...
	/**
	 * 设置 id header.
	 * 适用于SUBSCR0BE, UNSUBSCRIBE, 和ACK 或 NACK帧.
	 */
	...
	/**
	 * 设置ack header为"auto", "client", 或 "client-individual"之一.
	 * 适用于SUBSCRIBE 和 MESSAGE帧.
	 */
	...
	/**
	 * 返回只能读取但不能写入的{@code StompHeaders}对象.
	 */
