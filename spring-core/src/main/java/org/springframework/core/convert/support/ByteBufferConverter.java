...
/**
 * 将{@link ByteBuffer}直接转换为{@code byte[]},
 * 或从{@code byte[]}转换为{@link ConversionService}通过{@code byte[]}支持的任何类型.
 */
final class ByteBufferConverter implements ConditionalGenericConverter {
	...
	private Object convertToByteBuffer(Object source, TypeDescriptor sourceType) {
		...
		// 在JDK 9上编译以及在JDK 8上运行所需的额外强制转换,
		// 因为否则将选择在JDK 8上不可用的重写的ByteBuffer返回重绕方法.
