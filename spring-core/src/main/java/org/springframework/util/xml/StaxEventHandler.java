...
class StaxEventHandler extends AbstractStaxHandler {
	...
	/**
	 * 构造一个写入给定{@code XMLEventWriter}的{@code StaxEventContentHandler}的新实例.
	 * 将创建默认的{@code XMLEventFactory}.
	 * 
	 * @param eventWriter 要写入事件的写入器
	 */
	...
	/**
	 * 构造{@code StaxEventContentHandler}的新实例,
	 * 该实例使用给定的事件工厂来创建事件, 并写入给定的{@code XMLEventConsumer}.
	 * 
	 * @param eventWriter 要写入事件的写入器
	 * @param factory 用于创建事件的工厂
	 */
