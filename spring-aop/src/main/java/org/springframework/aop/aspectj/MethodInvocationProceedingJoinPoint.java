...
/**
 * AspectJ {@link ProceedingJoinPoint}接口的实现, 封装了一个AOP Alliance {@link org.aopalliance.intercept.MethodInvocation}.
 *
 * <p><b>Note</b>: {@code getThis()}方法返回当前Spring AOP 代理.
 * {@code getTarget()}方法返回当前Spring AOP目标 (如果没有目标实例, 则可能是{@code null}), 作为一个普通的POJO没有任何增强.
 * <b>如果要调用该对象并使增强生效, 使用{@code getThis()}.</b>
 * 一个常见的例子是在引入的实现中将对象转换为引入的接口. AspectJ本身在目标和代理之间没有这种区别.
 */
public class MethodInvocationProceedingJoinPoint implements ProceedingJoinPoint, JoinPoint.StaticPart {
	...
	/**
	 * 返回Spring AOP 目标. 可能是{@code null}, 如果没有代理.
	 */
	@Override
