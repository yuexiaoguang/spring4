...
/**
 * 包含Spring Expression Language可以生成的所有消息.
 * 每条消息都有一种 (info, warn, error) 和代码.
 * 可以编写测试以期望特定的代码而不是特定的文本, 从而可以更轻松地修改消息文本, 并使测试在不同的区域设置中成功运行.
 *
 * <p>当格式化消息时, 它将具有这种形式, 捕获前缀和错误类型:
 *
...
public enum SpelMessage {
	...
	/**
	 * 生成包含前缀和应用于消息的插入的完整消息.
	 * 
	 * @param inserts 要插入格式化消息的插入内容
	 * 
	 * @return 格式化的消息
	 */
	...
	/**
	 * 生成完整的消息, 包括前缀, 位置 (如果已知), 以及应用于消息的插入.
	 * 
	 * @param pos 位置 (忽略并且如果小于0则不包括在消息中)
	 * @param inserts 要插入格式化消息的插入内容
	 * 
	 * @return 格式化的消息
