...
/**
 * 索引器可以索引到一些进程结构以访问它的特定部分.
 * 支持的结构时: strings / collections (lists/sets) / arrays.
 */
// TODO 支持多维数组
// TODO 支持多维 [][][] 的正确语法, 而不是 [,,,]
public class Indexer extends SpelNodeImpl {
	...
	// 当索引器用作属性读取访问器时, 将使用这些字段.
	// 如果名称和目标类型与这些缓存值匹配, 则使用cachedReadAccessor读取属性.
	// 如果它们不匹配, 则会发现正确的访问器, 然后将其缓存以供以后使用.
	...
	// 当索引器用作属性写入访问器时, 将使用这些字段.
	// 如果名称和目标类型与这些缓存值匹配, 则使用cachedWriteAccessor写入属性.
	// 如果它们不匹配, 则会发现正确的访问器, 然后将其缓存以供以后使用.
	...
	protected ValueRef getValueRef(ExpressionState state) throws EvaluationException {
		...
		// if子句的第一部分阻止了属性的 '双重取消引用' (SPR-5847)
		if (target instanceof Map && (this.children[0] instanceof PropertyOrFieldReference)) {
			...
			// 如果映射键不合格, 希望它根据根对象进行评估, 因此在评估键时暂时将其推入
			try {
	...
	public void generateCode(MethodVisitor mv, CodeFlow cf) {
		...
		if (this.indexedType == IndexedType.ARRAY) {
			...
			// 键是一个不带引号的字符串文字的特殊情况, 它将被解析为属性/字段引用
