 ...
 * 这允许混合访问Hibernate的服务和使用普通JDBC的服务 (不知道Hibernate)!
 ...
 *
 * <p><b>NOTE: 强烈建议使用Hibernate 4.2+进行Spring的高效事务管理, 特别是对于事务性Spring JDBC访问.</b>
 */
...
public class HibernateTransactionManager extends AbstractPlatformTransactionManager
	 ...
	 * <p>如果SessionFactory配置了LocalDataSourceConnectionProvider,
	 * i.e. 通过Spring的LocalSessionFactoryBean与指定的"dataSource", 将自动检测DataSource:
	 * 仍然可以显式指定DataSource, 但在这种情况下不需要.
	 * <p>将此DataSource的事务JDBC连接提供给, 通过DataSourceUtils或JdbcTemplate直接访问此DataSource的应用程序代码.
	 * Connection将从Hibernate Session中获取.
	...
	/**
	 * 设置是否准备事务性Hibernate会话的底层JDBC连接,
	 * 即是否将特定于事务的隔离级别和/或事务的只读标志应用于底层JDBC连接.
	 * <p>默认"true". 如果关闭此标志, 事务管理器将不再支持每个事务的隔离级别.
	...
	/**
	 * 设置是否在Hibernate管理的Session操作, 而不是Spring管理的Session上操作,
	 * 即是否通过Hibernate的{@link org.hibernate.SessionFactory#getCurrentSession()}
	 * 而不是{@link org.hibernate.SessionFactory#openSession()}获取Session
	 * (在它之前检查 {@link org.springframework.transaction.support.TransactionSynchronizationManager}).
	 * <p>默认"false", i.e. 使用Spring管理的Session:
	 * 获取当前线程绑定的Session (e.g. 在Open-Session-in-View场景中), 否则为当前事务创建新的Session.
	 * <p>将此标志切换为"true" 以强制使用Hibernate管理的Session.
	 * 请注意, 这需要{@link org.hibernate.SessionFactory#getCurrentSession()}在调用Spring管理的事务时始终返回正确的Session;
	 ...
	 * 在这种情况下, 预绑定Session <i>不会</i>接收{@code clear()}调用 (在回滚时), 或{@code disconnect()}调用 (在事务完成时);
	...
	/**
	 * 返回当前的Hibernate实体拦截器, 则返回{@code null}.
	 * 如有必要, 通过bean工厂解析实体拦截器bean名称.
	 * 
	 * @throws IllegalStateException 如果指定了bean名称, 但没有设置bean工厂
	 * @throws BeansException 如果通过bean工厂的bean名称解析失败
	 */
	...
	protected void doCleanupAfterCompletion(Object transaction) {
		...
		if (this.prepareConnection && session.isConnected() && isSameConnectionForEntireSession(session)) {
			// 正在运行连接释放模式"on_close": 可以在这里重置JDBC连接的隔离级别和/或只读标志.
			// 否则, 需要依靠连接池来执行适当的清理.
			try {
				Connection con = ((SessionImplementor) session).connection();
	...
	/**
	 * 在事务完成时断开预先存在的Hibernate会话, 返回其数据库连接但保留其实体状态.
	 ...
	 * <p>默认实现将检查会话的连接释放模式是否为"on_close".
	 * 
