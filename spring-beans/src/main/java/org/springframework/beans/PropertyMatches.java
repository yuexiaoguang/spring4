...
/**
 * 用于计算属性匹配的帮助类, 根据可配置的距离.
 * 提供潜在匹配的列表以及生成错误消息的简便方法. 适用于java bean属性和字段.
 *
 * <p>主要用于框架内, 特别是绑定设施.
 */
public abstract class PropertyMatches {
	 ...
	 * 
	 * @param propertyName 查找可能匹配的属性名称
	 ...
	 * 
	 * @param propertyName 用于查找可能匹配的属性名称
	 * @param beanClass 用于搜索匹配的bean类
	 */
	...
	/**
	 * 为给定属性和可能的​​匹配创建一个新的PropertyMatches实例.
	 */
	...
	private static class BeanPropertyMatches extends PropertyMatches {
		...
		/**
		 * 为给定的属性和类生成可能的属性替代.
		 * 内部使用{@code getStringDistance}方法, 而这又使用Levenshtein算法来确定两个字符串之间的距离.
		 * 
		 * @param descriptors 要搜索的JavaBeans属性描述符
		 * @param maxDistance 接受的最大距离
		 */
